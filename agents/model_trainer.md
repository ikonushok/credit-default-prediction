...
- Sequence/neural models (GRU/Transformer) over the raw per-`id` product history — a different representation than aggregates, strong for ensemble diversity.

## Sequence / neural model rules

When training a sequence or neural model (instead of GBDT on aggregates):

- Use the SAME `id`-level `make_folds(seed)` as the GBDT runs so OOF/test align by `id` and are ensemble-able; store predictions in a canonical ascending-`id` order (do not leave them in file/appearance order).
- Treat the integer-encoded columns as categorical codes via embeddings; size each embedding from `max(train, test) + 1` so unseen test codes never index out of range.
- Standardize/scale numeric inputs with statistics fit on train only (label-free); never fit on the validation fold's labels.
- Early stopping on the fold's own validation only; no validation rows leak into the final fit.
- Determinism: fix `torch`/numpy seeds; note MPS/GPU non-determinism and float64 limits (MPS rejects float64 — use float32); report seed and device.
- Prefer multi-seed averaging over a single run for neural models; report seed variance.
- Pad/truncate sequences consistently for train and test; padded steps must be masked (e.g. `pack_padded_sequence`).
