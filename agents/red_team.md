...
- Could the score come from target leakage, future information, raw `id`/`rn` ordering, duplicated rows, or an `id` whose history rows were split across folds (split before aggregation)?
- Is the validation split representative of the platform test distribution, including the temporal `id`-range shift between train and test?
...
- What would fail if the platform split is temporal (later `id`s) and local CV is random?
- What would fail if aggregation was computed across `id`s, or the split was made on raw rows before aggregation?
- At a ≈3.55% positive rate, is the ROC-AUC stable across folds, or driven by a few positives?
