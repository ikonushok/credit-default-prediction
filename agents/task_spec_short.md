...
## Protected contracts
- no target/test leakage; `flag` joined by `id` from train_target.csv only
- preserve sample submission format (`id`, `flag`)
- full schema discovery (61 columns) before feature exclusion
- aggregate long history within `id`; split on `id`; check temporal (`id`-order) risk before trusting random CV
...
Task: <task>.
Inputs: <paths>.
Constraints: do not use target/test leakage; preserve sample_submission.csv contract; inspect full schema; aggregate within `id` and split on `id`; check temporal (`id`-order) risk; report achieved validation level.
