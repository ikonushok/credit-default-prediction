...
- Positive class is `flag = 1` (default).
- Validation rows are OOF/held-out at the `id` level; no `id` appears in both train and validation of a fold.
- OOF/test predictions are aligned to the target and to other runs **by `id` value, not by array position**. A run may store predictions in a different `id` order (e.g. file/appearance order vs ascending `id`); verify the stored `id` order matches the label vector before scoring or comparing.
...
- Predictions are aligned to labels by array position without verifying the `id` order matches (predictions stored in a different `id` order than the label vector).
- Test prediction row order is assumed but not verified against `sample_submission` `id`s.
- Reported ROC-AUC comes from a split that data-quality/leakage checks marked invalid (e.g. split before aggregation).
