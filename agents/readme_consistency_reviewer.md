...
- Scores include split type, `id`-level/time policy, folds/seeds, metric definition, and run id.
...
- No stale references to the unrelated "credit-offer acceptance" framing remain; the task is default prediction.

## Critical blocks

- README says a model/submission exists but artifact path is missing.
- README reports a score without validation context including `id`-level/time policy.
