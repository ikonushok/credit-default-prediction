...
Use first when a task can touch several parts of the Alfa credit-scoring workflow.
...
- `data_quality_review` — full schema discovery (61 columns), missingness, duplicates, long-format/`id` coverage, target distribution, train/test drift.
- `eda_review` — feature distributions, feature-default relationships, business interpretation.
- `leakage_review` — target leakage, test leakage, temporal leakage, ID leakage, aggregation leakage, encoder leakage.
- `feature_engineering` — per-`id` aggregate features, transformations, pipeline changes.
- `cv_design` — `id`-level split strategy, time policy, fold reproducibility, OOF predictions.
...
- Do not trust random CV until `id`-level splitting and temporal (`id`-order) risk are checked.
...
Expected output: ...
Schema / id-level / temporal checks required: yes/no
```
