...
| L2 | Data/schema consistency | train/test/sample schema, long-format/`id` coverage, missingness, duplicates, target sanity |
| L3 | Reproducible CV | saved `id`-level folds or deterministic splits, documented split/time policy, per-fold and OOF ROC-AUC |
...
- Data report: needs L2, including full schema discovery and long-format/`id`-coverage diagnostics.
...
- User asks which model is best but experiments use incompatible folds or split/time policies.
