...
- Top features are plausible for default probability.
- Delinquency, payment-status, and utilization aggregates behave in business-plausible directions or are explained.
- `id`/`rn`-derived, constant, or rare-category features are not dominating suspiciously.
- Categorical importances are not driven by tiny groups or rare-category artifacts.
...
- `id`, `rn`, or another raw identifier/ordering proxy dominates feature importance.
- A feature derived from `flag` or the sample submission appears important.
